...
def test_sensor_readings(csvpath):
    timestamps, values = tellsticklogger.sensor_readings(226, 2, 'oregon', '1a2d', csvpath=csvpath)
...
def test_get_sensors_location_error(empty_csvpath):
    with pytest.raises(tellsticklogger.LocationNotSetError) as excinfo:
        tellsticklogger.get_sensors_location(csvpath=empty_csvpath)
...
def test_get_sensor_location_error(csvpath):
    ...
    with pytest.raises(tellsticklogger.LocationNotSetError) as excinfo:
        tellsticklogger.get_sensor_location(181, csvpath=csvpath)

    assert 'sensor 181' in str(excinfo.value)


def test_get_sensor_location(csvpath):
    location = tellsticklogger.get_sensor_location(226, csvpath=csvpath)
...
def test_set_sensor_location(sensors, csvpath):
    ...
    tellsticklogger.set_sensor_location(sensor, csvpath=csvpath)
    assert sensor['location'] == tellsticklogger.get_sensor_location(226, csvpath=csvpath)
